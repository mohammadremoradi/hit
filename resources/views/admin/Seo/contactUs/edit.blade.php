    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <form action="{{ route('contact-us-page.update', $contact_us_page->id) }}" method="POST">
                            @method('put')
                            @csrf
                            <div class="row p-2">
                                @if ($errors->any())
                                ...
                                <div class="form-floating mb-3 c col-md-6">
                                    <input name="name" value="{{ old('name', $contact_us_page->name) }}" type="text"
                                ...
                                <div class="form-floating mb-5 col-md-4">
                                    <select dir="rtl" class="form-select" name="lang" id="floatingSelect"
                                        ...
                                        @foreach ($langs as $lang)
                                            <option @selected($lang == $contact_us_page->lang) value="{{ $lang }}">
                                                {{ $lang }}
                                            </option>
                                        @endforeach
                                ...
                                <div class="form-floating mb-5 col-md-4">
                                    <select dir="rtl" class="form-select" name="location" id="floatingSelect"
                                        aria-label="Floating label select example">
                                        <option @selected(old('location', $contact_us_page->location) == "header") value="header">
                                            header
                                        </option>
                                        <option @selected(old('location', $contact_us_page->location) == "script") value="script">
                                            script
                                        </option>
                                    </select>
                                    <label for="floatingSelect">Language</label>
                            ...
                            <div class="form-floating">
                                <textarea name="script" class="form-control" placeholder="Leave a comment here" id="floatingTextarea2"
                                    style="height: 100px">{{ old('script', $contact_us_page->script) }}</textarea>
                                @error('script')
